			...
			openApi
				...
				.forEach(pathItem -> pathItem
										...
										.forEach((httpMethod, operation) -> {
											...
											switch (httpMethod) {
												case GET: {
													apiResponses.addApiResponse("406", new ApiResponse().$ref(notAcceptableResponse));
													apiResponses.addApiResponse("500", new ApiResponse().$ref(internalServerErrorResponse));
													break;
												}
												case POST: {
													apiResponses.addApiResponse("400", new ApiResponse().$ref(badRequestResponse));
													apiResponses.addApiResponse("500", new ApiResponse().$ref(internalServerErrorResponse));
													break;
												}
												case PUT: {
